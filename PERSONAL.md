...
- **工程纪律**：固定模型路由、最小改动、行为不变量优先 —— 三 Agent 角色、严格提升才保留、回归守卫只更严
...
| **每轮并行变异** | 每轮同时生成多个隔离候选，并行复评取最优，严格提升才保留 |
...
- 生成暂时默认使用本机 Codex App Server + ChatGPT 登录；Qwen、DeepSeek、GLM 保留固定路由；兼容字段恒为 `qwen_api_key`
- 密钥只在组件内存与请求体中流动，绝不落日志 / 会话 / zip / git
- 临时默认模型 `gpt-5.6-sol`，可用兼容变量 `QWEN_MODEL` 覆盖

## 维护约定

见 [`AGENTS.md`](AGENTS.md)：行为不变量、验证流程（后端单测 + 前端构建 + 禁止依赖扫描 + 受影响模型路由真实冒烟）全部保留并扩展。
