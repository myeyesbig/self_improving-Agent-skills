...
- `backend/llm_client.py` is the thin model-call layer. It owns the fixed local Codex App Server route, direct DashScope SDK call, explicit `deepseek-` / `glm-` HTTP branches, exponential-backoff retry, timeouts, and JSON mode. Async callers bridge its synchronous calls with `asyncio.to_thread`.
...
- Do not add tools, RAG, MCP, databases, queues, authentication, or deployment infrastructure without explicit scope.
  ...
  - **Approved exception (2026-08-27, user-explicit)**: local single-user generation may use Codex App Server through the fixed `gpt-` model prefix and temporarily defaults to the account-advertised `gpt-5.6-sol`. It must use an existing ChatGPT login, verify the requested model with `model/list`, honor only advertised reasoning efforts, and reject API-key auth. Each call uses a fresh ephemeral thread in an empty temporary cwd with read-only/no-network/no-approval settings and fails on any tool item. Structured JSON uses App Server `outputSchema` with a strict string wrapper that is unwrapped before existing Pydantic validation. It may not call the OpenAI Platform API, consume `OPENAI_API_KEY`, introduce the OpenAI SDK, persist Codex threads, or generalize provider selection. The compatibility request field remains `qwen_api_key` and may be empty only for the Codex route. OpenAI embedding models are not part of this exception: lesson semantic retrieval remains DashScope `text-embedding-v3` using `DASHSCOPE_API_KEY` or degrades to the existing local/tag path.
