...
SkillForge temporarily defaults generation to the local Codex App Server using ChatGPT subscription authentication and the account-advertised `gpt-5.6-sol` model. This is a fixed `gpt-` route, not the OpenAI Platform API and not a generic provider abstraction. The existing Qwen-primary DashScope implementation remains the fallback for non-`gpt-`/`deepseek-`/`glm-` model names; DeepSeek and Zhipu GLM remain the two approved optional HTTP alternatives. The optimization loop is orchestrated by LangGraph. Do not add Gemini, Google ADK, OpenAI Platform API-key generation, Anthropic, Ollama, vLLM, or generic provider abstractions unless the user explicitly expands the scope. LangChain (`langchain`/`langchain-community`) must not be imported in code.
...
- `backend/llm_client.py` is the thin model-call layer. It owns the fixed local Codex App Server route and its lazy warm worker pool, direct DashScope SDK call, explicit `deepseek-` / `glm-` HTTP branches, exponential-backoff retry, timeouts, and JSON mode. Async callers bridge its synchronous calls with `asyncio.to_thread`.
...
- The roles may use different models through `EXECUTOR_MODEL`, `ANALYST_MODEL`, and `MUTATOR_MODEL`. Each variable defaults to the compatibility variable `QWEN_MODEL`, so unset role overrides preserve one shared model. A `gpt-` role uses only Codex ChatGPT auth; `deepseek-` and `glm-` retain their explicit routes; every other model uses DashScope.
...
- The temporary default model is `gpt-5.6-sol`; the compatibility variable `QWEN_MODEL` may override it on the backend, and role-specific variables may override it per role.
- Codex is selected only by the fixed `gpt-` prefix and must call the local App Server. DashScope, DeepSeek, and Zhipu GLM retain their fixed routes; no runtime provider registry or general OpenAI-compatible provider layer is allowed.
- Codex App Server connections are reused through a lazy LIFO worker pool (`CODEX_APP_SERVER_POOL_SIZE`, default/max 3): sequential calls reuse one initialized process while concurrent candidates may occupy additional workers. Every LLM call still creates a new ephemeral thread, rechecks `account/read` and `model/list`, and routes notifications by `threadId`/`turnId`; no model conversation history may be reused.
- Codex subprocess calls, DashScope SDK calls, and DeepSeek/GLM HTTP calls are synchronous and must run through `asyncio.to_thread` (via `llm_client`) when invoked from async FastAPI code.
...
- The Codex route must call `account/read` before submitting prompts and accept only `account.type=chatgpt`; API-key authentication must fail closed so subscription usage cannot silently become Platform API billing.
- Codex turns must be ephemeral, read-only, network-disabled, approval-free, and text-only. Any command, file, MCP, web, image, or other tool item aborts the call. Remove OpenAI/DashScope/DeepSeek/GLM API-key variables from the child environment.
- When Codex generation is selected, SKILL.md text, scenarios, eval criteria, execution outputs, and generation prompts are transmitted to OpenAI under the signed-in ChatGPT workspace's data controls. Do not persist them as Codex threads or add them to SkillForge logs/lesson stores.
...
- Do not add tools, RAG, MCP, databases, queues, authentication, or deployment infrastructure without explicit scope.
  ...
  - **Approved exception (2026-08-27, user-explicit)**: local single-user generation may use Codex App Server through the fixed `gpt-` model prefix and temporarily defaults to the account-advertised `gpt-5.6-sol`. It must use an existing ChatGPT login, verify the requested model with `model/list`, honor only advertised reasoning efforts, and reject API-key auth. App Server processes may stay warm in a bounded lazy pool (default/max 3), but each call must still use a fresh ephemeral thread in an empty worker-owned temporary cwd with read-only/no-network/no-approval settings, route events by `threadId`/`turnId`, and fail on any matching tool item. Broken/failed workers must be reset before retry. Structured JSON uses App Server `outputSchema` with a strict string wrapper that is unwrapped before existing Pydantic validation. It may not call the OpenAI Platform API, consume `OPENAI_API_KEY`, introduce the OpenAI SDK, persist Codex threads, reuse conversation history, or generalize provider selection. The compatibility request field remains `qwen_api_key` and may be empty only for the Codex route. OpenAI embedding models are not part of this exception: lesson semantic retrieval remains DashScope `text-embedding-v3` using `DASHSCOPE_API_KEY` or degrades to the existing local/tag path.
...
4. For changes touching model execution, perform a live `max_rounds=1` smoke test against every affected route. Codex/default-route changes require a ChatGPT-authenticated Codex smoke; DashScope, DeepSeek, or GLM branch changes require separate smokes for those affected routes. One provider's result must not be presented as verification of another; if affected authentication is unavailable, report that route as unverified.
